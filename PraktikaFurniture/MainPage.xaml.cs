...
namespace PraktikaFurniture
{
    public partial class MainPage : Page
    {
        ...
        {
            var currentProducts = DB.dbContext.Products.ToList();

            if (string.IsNullOrWhiteSpace(SearchTextBox.Text) == true)
                SearchTextBox.Text = "";
            else
                currentProducts = currentProducts.Where(u => u.ProductName.ToLower().Contains(SearchTextBox.Text.ToLower())).ToList();
        ...
        private void GlobalUnhandledExceptionHandler(object sender, UnhandledExceptionEventArgs e)
        {
            ...
            MessageBox.Show("Поддержка предупреждена об ошибке.\nСкоро мы начнем разбираться в причинах ошибки.", "Обнаружена ошибка!", MessageBoxButton.OK, MessageBoxImage.Error);
            try
            {
        ...
        private void MenuItem_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                using (var templateDoc = DocX.Load(Assembly.GetExecutingAssembly().GetManifestResourceStream("PraktikaFurniture.doc-template.docx")))
                {
                    ...
                    int sumQuant = 0;
                    foreach (var item in ListViewProducts.SelectedItems) sumQuant += (item as Product).StockQuantity;
